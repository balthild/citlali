// These codes are copied from WXT
// See: https://github.com/wxt-dev/wxt/blob/main/packages/wxt/src/core/utils/transform.ts

import { parseModule, ProxifiedModule } from 'magicast';
...
function emptyMainFunction(mod: ProxifiedModule): void {
    if (mod.exports?.default?.$type === 'function-call') {
        if (mod.exports.default.$ast?.arguments?.[0]?.body) {
            ...
            // ex: "fn({ ..., main: () => {} })" to "fn({ ... })"
            mod.exports.default.$ast.arguments[0].properties = mod.exports.default.$ast.arguments[0].properties
                .filter((prop: any) => prop.key.name !== 'main');
        }
    }
}
